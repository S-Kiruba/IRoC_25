...
class DroneController:
    def __init__(self, connection_port: str, baudrate: int):
        ...
        try:
            ...
            self.drone = connect(connection_port, baud=baudrate, wait_ready=True, timeout=120)
    ...
    def lift_to_distance(self, altitude: float, timeout: int = 10, throttle: int = 1650):
    ...
    def hover(self):
        ...
        current_alt = self.drone.rangefinder.distance
        if current_alt < 0.15:
            logger.info('the drone on the land')
            return
        else:
    ...
    def land(self):
        ...
        if not self.drone.armed:
            logger.info("Drone is disarmed. Skipping landing.")
            return

        # Safety: Confirm rangefinder is available
        if self.drone.rangefinder is None or self.drone.rangefinder.distance is None:
            logger.warning("Rangefinder data unavailable. Cannot perform safe landing.")
            return

        logger.info("Initiating landing sequence using throttle 1350...")

        # Continue applying throttle until close to ground (e.g., < 0.2 m)
        while True:
            altitude = self.drone.rangefinder.distance
            if altitude is not None:
                logger.info("Current altitude: %.2f m", altitude)
                if altitude <= 0.15:
                    logger.info("Drone is on the ground. Landing complete.")
                    break
            else:
                logger.warning("Altitude reading not available.")

            # Apply throttle value of 1250
            self.drone.channels.overrides = {'3': 1400}
            time.sleep(0.1)
...
def main(args=None):
    ...
    # node.destroy_node()
    # rclpy.shutdown()
    drone = DroneController("/dev/ttyUSB0",57600)
    drone.arm()
    time.sleep(2)
    drone.lift_to_distance(1.2)
    print("Lifted..")
    # for i in range(5):
    # drone.hover()
    # time.sleep(5
